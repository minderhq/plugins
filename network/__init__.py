...
class NetworkPlugin:
    ...
    # is a JSON Schema. Aggregated by GET /v1/plugins/ai/tools.
    AI_TOOLS = [
        {
            "name": "network_scan",
            "description": (
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # #984 (multi-tenant SaaS spine): LAN-scan output is real, private infra
        # data (hosts/MACs/open ports), unlike the public crypto/news series -- so
        # every sink is tenant-tagged. This singleton plugin scans on behalf of the
        # organization that owns the deployment's network monitoring; its tenant is
        # configured here (falls back to the default org for a single-tenant dev
        # stack). All three sinks stamp + scope by it so two tenants' scans of
        # overlapping IP ranges never collide or leak across each other.
        self.tenant_id = os.environ.get("NETWORK_TENANT_ID", "") or os.environ.get(
            "MINDER_DEFAULT_TENANT_ID", ""
        )
    ...
    async def _sink_telegraf(self, live: List[Dict]) -> Dict:
        ...
        cfg = _telegraf_config(live, self.snmp_community, self.tenant_id)
    ...
    async def _sink_postgres(self, live: List[Dict], changes: Dict) -> Dict:
        ...
        try:
            ...
            return {"status": "error", "error": type(e).__name__}
        try:
            # #984 / Increment 6: this plugin owns its OWN short-lived connection
            # (not the pooled one carrying the RLS `setup` callback), so stamp the
            # session GUCs here. The plugin instance is single-tenant (self.tenant_id),
            # so the RLS backstop below enforces exactly this tenant's rows on top of
            # the explicit WHERE tenant_id=... predicates.
            await conn.execute(
                "SELECT set_config('app.tenant_id', $1, false), "
                "set_config('app.is_platform', 'off', false)",
                self.tenant_id,
            )
            create_table_sql = """CREATE TABLE IF NOT EXISTS network_inventory (
            ...
            # the pre-tenant scope), then repoint the PK.
            await conn.execute(
                "ALTER TABLE network_inventory "
                "ADD COLUMN IF NOT EXISTS tenant_id TEXT NOT NULL DEFAULT ''"
            )
            await conn.execute(
                "ALTER TABLE network_inventory DROP CONSTRAINT IF EXISTS "
                "network_inventory_pkey"
            )
            await conn.execute(
                "ALTER TABLE network_inventory " "ADD PRIMARY KEY (tenant_id, ip)"
            )
            # Increment 6: RLS backstop on the runtime-created inventory table. The
            # plugin sets app.tenant_id above; FORCE so the owner role is subject.
            await conn.execute(
                "ALTER TABLE network_inventory ENABLE ROW LEVEL SECURITY"
            )
            await conn.execute("ALTER TABLE network_inventory FORCE ROW LEVEL SECURITY")
            await conn.execute(
                "DROP POLICY IF EXISTS tenant_isolation ON network_inventory"
            )
            await conn.execute(
                "CREATE POLICY tenant_isolation ON network_inventory "
            ...
            for h in live:
                ...
                await conn.execute(
                    """INSERT INTO network_inventory
                         (tenant_id, ip, hostname, os, ports, snmp, status, last_seen)
                       VALUES ($1,$2,$3,$4,$5,$6,'up',NOW())
                       ON CONFLICT (tenant_id, ip) DO UPDATE SET
                         hostname=$3, os=$4, ports=$5, snmp=$6,
            ...
            # Reconcile within THIS tenant only: another tenant's hosts must never be
            # marked down by this tenant's scan (#984). Then purge stale-down rows.
            live_ips = [h.get("host", "") for h in live if h.get("host")]
            down = await conn.execute(
                "UPDATE network_inventory SET status='down' "
                "WHERE tenant_id=$2 AND NOT (ip = ANY($1::text[])) AND status <> 'down'",
                live_ips,
                self.tenant_id,
            )
            purged = await conn.execute(
                "DELETE FROM network_inventory WHERE tenant_id=$2 AND status='down' "
    ...
    async def _sink_neo4j(self, live: List[Dict]) -> Dict:
        ...
        # #984: tenant-scope every topology node. tenant_id is part of the MERGE key
        # (Host {ip, tenant_id}, Service/Interface {key, tenant_id}) so two tenants
        # scanning the same IP get DISTINCT nodes that never merge, and it's SET on
        # each so a later per-tenant prune can target only its own subgraph.
        statement = (
            "UNWIND $hosts AS h "
            "MERGE (host:Host {ip: h.host, tenant_id: $tenant}) "
            "SET host.hostname=h.hostname, host.os=h.os, host.last_seen=timestamp() "
            "FOREACH (p IN h.ports | "
            "  MERGE (s:Service {key: h.host + ':' + toString(p), tenant_id: $tenant}) "
            "  SET s.port=p "
            "  MERGE (host)-[:RUNS]->(s)) "
            "FOREACH (nm IN h.ifaces | "
            "  MERGE (i:Interface {key: h.host + ':' + nm, tenant_id: $tenant}) "
            "  SET i.descr=nm "
            "  MERGE (host)-[:HAS_INTERFACE]->(i))"
        )
        # Prune stale topology WITHIN this tenant only (#984): another tenant's Hosts
        # must never be pruned by this tenant's scan. Orphaned Service/Interface
        # cleanup stays global (an orphan has no owning Host regardless of tenant).
        retention_ms = self.stale_retention_days * 86400 * 1000
        prune = [
            {
                "statement": "MATCH (h:Host) WHERE h.tenant_id = $tenant AND "
                "h.last_seen < timestamp() - $ms DETACH DELETE h",
                "parameters": {"ms": retention_ms, "tenant": self.tenant_id},
            },
            {
                "statement": "MATCH (s:Service) WHERE NOT EXISTS { (s)<-[:RUNS]-() } DELETE s"
            },
            {
                "statement": "MATCH (i:Interface) WHERE NOT EXISTS "
                "{ (i)<-[:HAS_INTERFACE]-() } DELETE i"
            },
        ]
        try:
    ...
    async def analyze(self) -> Dict:
        ...
        return {
            ...
            "telegraf_config": _telegraf_config(
                live, self.snmp_community, self.tenant_id
            ),
        }
